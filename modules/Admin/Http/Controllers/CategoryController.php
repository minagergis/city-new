...
class CategoryController extends AdminController
{
    ...
    public function getCategoryProperties()
    {
        ...
        $properties = CategoryTranslation::join('category', 'category.id', '=', 'category_translations.category_id')
            ->select([
        ...
        foreach ($this->language_available as $l) {
            $datatables->addColumn($l, function ($properties) use ($l) {

                $c = CategoryTranslation::where('category_id', $properties->category_id)
                    ->where('locale', $l)
                    ->get();
                if (count($c) > 0) {
                    if ($properties->locale == $l) {
                        $icon = 'ok';
                    } else {
                        $icon = 'pencil';
                    }
                    return '<a href="' . route('admin.category.edit.get', [$properties->category_id, "&lang=$l"]) . '"><span class="glyphicon glyphicon-' . $icon . '"></span></a>';
                } else {
                    return '<a href="' . route('admin.category.add.get', "id=$properties->category_id&lang=$l") . '"><span class="glyphicon glyphicon-plus"></span></a>';
                }

            });
        }

        $datatables->addColumn('action', function ($properties) {
            return '<a class="btn btn-danger btn-sm" href="' . route('admin.category.delete.post', $properties->category_id) . '"><i class="fa fa-trash"></i>delete</a>';
        });
        return $datatables->make(true);

    }

    public function getCategoryGallary()
    {
        $lang = \Session::get('lang');
