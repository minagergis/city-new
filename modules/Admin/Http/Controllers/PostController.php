...
class PostController extends AdminController
{
    ...
    public function getAddPost()
    {
        ...
        $current_lang = $this->current_lang;
        $users = User::all();
        if ($action == 'add_translate') {
            $categories_ids = Post::join('post_category', 'posts.id', '=', 'post_category.post_id')
                ->select(['post_category.category_id as id'])
                ->where('post_id', $id)
                ->lists('id')
                ->toArray();
            return view('admin::sections.posts.create_edit', compact('post', 'categories_ids', 'post_type', 'language_icon', 'language_available', 'categories', 'language', 'current_lang', 'post', 'action', 'users'));
    ...
    protected function processForm($request, $id = null)
    {
        ...
        /*============== Translatable extra section Start ===============*/
        if ($request->has('trans_extra')) {
            $data_trans = [];
            foreach ($request->get('trans_extra') as $trans_key => $trans_extra) {
                $data_trans[$trans_key] = $trans_extra;
            }
            $post->translateOrNew($locale)->post_trans_extra = json_encode($data_trans);
        }
        /*============== Extra section End ===============*/
